...
#define inline
static inline uint32_t __builtin_bswap32(uint32_t in) { return ((in >> 24) & 0xFF) | ((in >> 8) & 0xFF00) | ((in & 0xFF00) << 8) | ((in & 0xFF) << 24); }
#endif
...
void texture_glyph_delete(texture_glyph_t* self)
{
  int i;
  assert(self);
  for (i = 0; i < self->kerning->size; i++) free(*(float**)vector_get(self->kerning, i));
...
float texture_glyph_get_kerning(const texture_glyph_t* self, const char* codepoint)
{
  ...
  assert(self);
  if (ucodepoint == -1) return 0;
  if (self->kerning->size <= i) return 0;
...
void texture_font_generate_kerning(texture_font_t* self, FT_Library* library, FT_Face* face)
{
  ...
  GLYPHS_ITERATOR(i, glyph, self->glyphs)
  {
    ...
    for (k = 0; k < glyph->kerning->size; k++) free(*(float**)vector_get(glyph->kerning, k));
...
int texture_font_set_size(texture_font_t* self, float size)
{
  ...
  if (0) {
    ...
    if (freetype_gl_warnings) log_error("selected: size[%i] for %f\n", best_match, size);
...
static int texture_font_init(texture_font_t* self)
{
  ...
  if (texture_font_load_face(self, self->size)) return -1;
...
texture_font_t* texture_font_clone(texture_font_t* old, float pt_size)
{
  ...
  if (!texture_font_set_size(self, pt_size)) return NULL;

  texture_font_init_size(self);

  if (self->size / self->scale != native_size) self->glyphs = vector_new(sizeof(texture_glyph_t*));
  return self;
}
  ...
  if (!self->face) {
    switch (self->location) {
      ...
      case TEXTURE_FONT_MEMORY:
        error = FT_New_Memory_Face(self->library->library, self->memory.base, self->memory.size, 0, &self->face);
        if (error) {
          freetype_error(error);
          goto cleanup_library;
        }
    ...
    if (texture_font_set_size(self, size)) goto cleanup_face;
  }
...
texture_glyph_t* texture_font_get_glyph(texture_font_t* self, uint32_t codepoint)
{
  ...
  if (!(glyph = texture_font_find_glyph(self, codepoint))) /* Glyph has not been already loaded */
    if (texture_font_load_glyph(self, codepoint)) glyph = texture_font_find_glyph(self, codepoint);
...
uint32_t texture_font_load_glyph_gi(texture_font_t* self, uint32_t glyph_index, uint32_t codepoint)
{
  ...
  glyph->codepoint = glyph_index ? codepoint : 0;
  ...
  int free_glyph = texture_font_index_glyph(self, glyph, codepoint);
  if (!glyph_index) {
    if (!free_glyph) {
  ...
  if (self->rendermode != RENDER_NORMAL && self->rendermode != RENDER_SIGNED_DISTANCE_FIELD) FT_Done_Glyph(ft_glyph);
